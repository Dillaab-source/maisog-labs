...
// Website Redesign V1 (D-076 / ML-DEVOS-AS-104): the separate local static
// spatial copy document (data/site.js `spatialContent`). Local schema only;
// not part of the legacy content document or any D1 model.
const spatialSchema = {
  entryDescriptor: text(400),
  destinations: [{ route: choice("systems", "projects", "research", "contact"), text: text(120) }],
  disciplines: [{ id, label: text(40), text: text(300), terms: [text(60)] }],
  contactStatement: text(120),
};

export function validateSpatialContent(document) {
  const errors = collectErrors(document, spatialSchema, "spatial");
  if (Array.isArray(document?.destinations) && document.destinations.map(item => item?.route).join() !== "systems,projects,research,contact") {
    errors.push("spatial.destinations: must list systems, projects, research, contact exactly once in that order");
  }
  if (errors.length) throw new Error(`Invalid spatial content:\n${errors.join("\n")}`);
  return document;
}
