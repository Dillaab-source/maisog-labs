...
// pointer update that moves only draft_revision_id, and one success audit
// row. Never touches published_revision_id and never UPDATEs an existing
// revision row's content (AS20-F005). The caller is responsible for the
// stale-pointer pre-check (AS20-F006) before calling this — this function
// assumes the caller has already confirmed expectedPublishedRevisionId/
// expectedDraftRevisionId match the current stored pointers.
export async function buildEditDraftBatch(db, { id, fields, createdAt, createdBy, actor }) {
  validateProjectRevisionContent(fields);
  const revisionColumns = projectRevisionColumns(fields);
  const revisionColumnNames = Object.keys(revisionColumns);
  const revisionValues = Object.values(revisionColumns);
  ...
  return [
    ...
    db
      .prepare(
        "UPDATE projects SET draft_revision_id = " +
          "(SELECT id FROM project_revisions WHERE project_id = ? AND revision_number = ?) WHERE id = ?"
      )
      .bind(id, revisionNumber, id),
    buildProjectRevisionAuditStatement(db, {
      actor,
...
// (AS20-F007).
export function buildPublishBatch(db, { id, draftRevisionId, actor }) {
  return [
    db
      .prepare("UPDATE projects SET published_revision_id = ?, draft_revision_id = NULL WHERE id = ?")
      .bind(draftRevisionId, id),
...
// (AS20-F008).
export function buildUnpublishBatch(db, { id, publishedRevisionId, actor }) {
  return [
    db.prepare("UPDATE projects SET published_revision_id = NULL WHERE id = ?").bind(id),
    buildAuditAppendStatement(db, {
      actor,
