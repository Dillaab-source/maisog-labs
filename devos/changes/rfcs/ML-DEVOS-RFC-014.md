...
| Sensitive implementation detail | **This repository's own storage, conditionally** (per the corrected `T10`/`ML-DEVOS-AS-048` storage model: only where classification, accepted access controls, Git suitability, and canonical-destination authorization all permit it; otherwise `STOP / DEFER PERSISTENCE` or route to an approved non-Git private/secret destination when one exists; `SECRET`/version-control-prohibited material never goes to Git regardless) | Corrected this cycle (`AS49-F006`) — a non-public-surfaced path is never, by itself, an accepted storage boundary. `RISK-WEB-013` remains open pending its own separate governed reassessment following the repository's visibility change (`D-041`/`ML-DEVOS-AS-047`); this discovery does not claim to resolve it |
...
|---|---|---|
| ChatGPT product skill surface | Product/plugin-based skill exposure (standalone Skills in the ChatGPT desktop app/IDE extension; plugin-distributed Skills across ChatGPT web/desktop/mobile — official evidence, §7), not a repository filesystem path | No — the official source that confirms the ChatGPT Skills feature exists does not document any arbitrary-repository filesystem discovery mechanism for it (unlike Codex CLI's `.agents/skills/`); this is a structurally different distribution question, not a fifth "vote" in a repo-path comparison (`AS45-O001`) |
...
| ChatGPT product skill exposure | OpenAI official documentation ("Build skills"), verified via search-engine-summarized excerpt of the official page — a direct fetch of `developers.openai.com`/`learn.chatgpt.com` was attempted this cycle (`AS49-F007` remediation) and blocked by this session's network egress policy, consistent with the same limitation already disclosed for GitHub Copilot and the primary Gemini CLI URL | `https://developers.openai.com/codex/skills` (redirects to the current "Build skills" documentation) | 2026-09-20 | Standalone Skills are available in the ChatGPT desktop app, Codex CLI, and the IDE extension; Skills bundled in plugins are additionally available in Chat and Work across ChatGPT web/desktop/mobile; both ChatGPT and Codex use progressive disclosure. **This source does not establish any arbitrary-repository-path filesystem discovery mechanism for the ChatGPT product surface itself** — Codex CLI's repo-local `.agents/skills/` discovery (row above) remains the only filesystem-path mechanism this evidence table supports. Corrected this cycle (`AS49-F007`) from a prior, now-stale "no official source found" statement — the gap was in the evidence gathered, not in the underlying ChatGPT Skills feature's existence. | OFFICIAL (source is the official page; verification method for this cycle was indirect, not a direct fetch — same disclosed limitation as the Copilot/Gemini rows above) |
...
17. **Unaccepted/unknown access controls near-miss** (from `ML-DEVOS-AS-048`'s clarification) — an otherwise-qualifying `INTERNAL` or `RESTRICTED` candidate whose access-control acceptance status is missing or uncertain still stops before persistence; private visibility alone never substitutes for that acceptance.
...
Yes — `ARCHITECTURE` class always requires one. This is the fourth returned revision of this discovery: the original (reviewed by `ML-DEVOS-AS-042`), the Remediation Cycle 1 revision (integrating `AS-042`/`AS-043`/`AS-044`, reviewed by `ML-DEVOS-AS-045`/`AS-046`/`AS-047`/`AS-048`), the Remediation Cycle 2 revision (reviewed by `ML-DEVOS-AS-049`), and this Remediation Cycle 3 (final consistency cleanup) revision.
