...
The kernel's own record — **Task Engine State** — is explicitly scoped to exactly: `state` (current lifecycle value), `owner`, `owner_generation` (fencing token), `lease_expires_at`, `expected_revision` (optimistic-concurrency counter), per-transition-class `retry_count`s, and a bounded internal transition history (see "Persistence" below). It is **not**:

- `coordination/STATE.md` — that file remains the bootstrap **turn-lock** signal between Paulo/Architect/Claude at the repository level; it is not a per-task state record and this RFC does not migrate, replace, or read/write it. **No migration of live coordination is authorized or proposed.**
...
The kernel's state vocabulary is exactly the frozen `ML-DEVOS-ARCH-001` §10 lifecycle, **plus two additive terminal states** this RFC proposes: `FAILED` and `ABANDONED`. §10's diagram has no terminal exit for a task that fails unrecoverably or is deliberately cancelled — without one, a kernel enforcing "no skipped transitions" would have no legal way to close such a task at all. This is flagged explicitly under "Affected rules" below as a proposed additive amendment to a `FROZEN` document, requiring the Architect's explicit sign-off rather than being treated as self-evidently in scope.
...
- **Single-owner claim.** At most one active owner per task at any time. `claim(task_id, actor_id, lease_duration)` succeeds only if no current owner exists, or the current lease has expired (`lease_expires_at < now`).
- **Fencing token (`owner_generation`).** Every successful `claim` atomically increments a monotonic `owner_generation` counter and persists the new owner alongside it, in one atomic write (see "Persistence"). Every subsequent `renew`/`release`/`transition` call must present the `owner_generation` the caller last observed; the kernel performs an atomic compare-and-swap against the persisted value and **rejects outright** (never merges, never partially applies) on any mismatch.
- **Expiry does not by itself invalidate a superseding claim's protection.** This is the specific race this RFC must close: expiry alone gates whether a *new* claim is permitted — it does **not** retroactively let a since-superseded old owner overwrite a newer owner's work. Because every mutating call must present the `owner_generation` it believes current, an old owner whose lease has expired (even one with a skewed clock still believing it holds the lease) can never succeed a write once a new claimant's `claim` has incremented `owner_generation` — the compare-and-swap will already fail against the stale generation it presents, independent of clock state.
- **Concurrent claimant conflict.** Two actors racing to claim the same expired lease: the persistence layer's atomic write (see below) guarantees only one `claim` can win the increment-and-persist step; the loser receives an explicit conflict result (never a silent no-op or a merged partial claim) and must re-fetch current state before retrying.
- **`renew(task_id, actor_id, owner_generation)`** extends `lease_expires_at` only if the presented `actor_id`/`owner_generation` matches the persisted record; it never changes `owner_generation` itself.
- **`release(task_id, actor_id, owner_generation)`** clears ownership without changing `state`; also fencing-guarded.
...
- **Idempotency key scope.** Each mutating request (`claim`/`transition`) carries a caller-supplied `idempotency_key`, scoped to `(task_id, operation_class)` — e.g., task `T`'s `BUILDING → READY_FOR_QA` transition attempt `K`.
- **Replay vs. conflicting reuse.** If the kernel finds a persisted record of the same `idempotency_key` for the same `(task_id, operation_class)` with **identical** request parameters (same `from`/`to`/`evidence_ref` content hash), it returns the previously committed result without re-applying the transition — this is the safe case (a network retry after a successful-but-unacknowledged write). If it finds the same key with **differing** parameters, this is a conflicting reuse and is rejected outright as a client error; the kernel never silently overwrites a prior committed result with a different one under the same key.
- **Durable, bounded retry counters.** Per-transition-class counters (`build_retry_count`, `qa_retry_count`, `review_retry_count`) persist across process restarts. The ceiling these counters compare against **reuses existing policy rather than inventing a new number**: it defaults to the live value of `coordination/STATE.md`'s existing `MAX_REMEDIATION_CYCLES` convention (currently `3`, Paulo-adjustable) at the time the ceiling is read, generalizing a convention Sentinel already applies to Architect-remediation cycles rather than creating a second, S4-specific retry-authority concept. Exceeding the ceiling forces a transition to `FAILED` or `PAULO_DECISION_REQUIRED` (see the transition table) rather than looping silently or indefinitely — mirroring the existing `ARCHITECT_SYNC.md` rule that an exceeded cap moves control to `PAULO_DECISION_REQUIRED`, never to unbounded autonomous looping.
...
- **Human escalation.** `PAULO_DECISION_REQUIRED` exists precisely to route ambiguous outcomes (a reviewer's explicit flag, or a retry ceiling exceeded in a way that might warrant Paulo raising the cap rather than failing the task) to a human, consistent with the existing `ARCHITECT_SYNC.md` remediation-cap behavior.
- **Crash-before-persist.** If an actor crashes before the kernel's write commits, no state change is observable (the persistence layer's atomicity guarantee, below, ensures there is no partially-applied write) — the caller's retry, using the same `idempotency_key`, is always safe because nothing was ever recorded as committed.
- **Crash-after-persist (before acknowledgment).** The write is durable; the caller's retry with the same `idempotency_key` finds the already-committed record and receives the identical result (the replay case above) — the caller never needs to know whether its original request actually landed on the far side of the crash.
- **Partial writes / corruption.** The chosen persistence approach (below) guarantees that a single task's state update is all-or-nothing — no torn/half-written record is ever observable by a reader. A corrupted on-disk record is detected at load time (structural/schema validation) and surfaces as an explicit, scoped error for that one `task_id` — never a silent default to some assumed state, and never a crash of the whole kernel process.
- **Restart.** The kernel holds no state only in memory; a full process restart simply re-opens the persistence layer and reads whatever was last durably committed. There is no separate "in-flight transition" concept that needs its own recovery procedure beyond re-reading current state — this is a stated design constraint, not an incidental property.
- **Duplicate delivery.** Exactly the case the idempotency key exists to solve (the replay case above).

### F. Persistence — comparison and recommendation

Four options were compared:

| Option | Atomicity | Portability | Recovery simplicity | Dependency cost | Fit for "smallest kernel" |
|---|---|---|---|---|---|
| **1. One local JSON file per task, write-temp-then-atomic-rename** | POSIX `rename()` is atomic on the same filesystem; a reader never observes a half-written file | High — Node built-ins only (`node:fs`), no external service | High — restart = re-read the file; a stray `.tmp` file is simply ignored/cleaned, never mistaken for committed state | Zero third-party dependencies | Best fit |
...
| **3. Local append-only event log (event-sourced)** | Atomic appends are simple | High | Lower — reading "current state" requires replaying the log or maintaining a snapshot alongside it | Zero third-party dependencies | More complexity than the *smallest* kernel needs; naturally suits a later S11 Memory & Observability enhancement instead |
...
**Recommendation: option 1** — one local JSON file per task, written via write-to-temp-file-then-atomic-rename, zero third-party dependencies (Node built-ins only: `node:fs`, `node:crypto` for idempotency-key hashing). This continues the same dependency-light posture Sentinel already established for Traceability V1 (`AS37-F010`) and S3's task-contract validator ("zero third-party deps"), keeps the corruption blast radius to a single task (one file, not one shared file for every task), and keeps the kernel small enough to match "the smallest authoritative task-state kernel" the acceptance criteria ask for.
...
Referenced, unmodified: `devos/contracts/` (S3 schema/validator, read-only reference), `devos/governance/rules/core-rules.json` (`CORE-001/002/006/007/012/014/016/017/018/020`, read-only reference), `brain/protocols/ARCHITECT_SYNC.md` (the existing `MAX_REMEDIATION_CYCLES` convention this RFC's retry-ceiling design reuses, read-only reference).
...
Mitigation: the fencing-token (`owner_generation`) compare-and-swap design in §D is specifically constructed so expiry alone can never let a superseded owner's write land; this is the single most safety-critical property of the whole design and should receive the most scrutiny in Architect review and, later, in test coverage.

### Retry ceiling reuse drifting from its original meaning
Mitigation: explicitly named as reuse of the existing `MAX_REMEDIATION_CYCLES` convention, read live rather than hardcoded, so a future Paulo change to that cap is inherited rather than requiring a second, S4-specific constant to be kept in sync by hand.
...
Compatible with: the frozen `ML-DEVOS-ARCH-001` §10/§11 (subject to the explicit two-terminal-state addition flagged above), the active S3 Task Contract mechanism (referenced, not duplicated), `CORE-001/002/006/007/012/014/016/017/018/020` (all consumed unchanged), the existing `MAX_REMEDIATION_CYCLES` convention (reused, not replaced), and Traceability V1 (which may index this RFC's and any future implementation's governance IDs without owning their semantics).
...
| 3 | Single-owner claim, fencing | `claim`/`renew`/`release` + `owner_generation` compare-and-swap (§D) | `devos/state/lease.mjs` | **Race test**: two concurrent `claim()` calls against one expired lease — assert exactly one wins, the loser gets an explicit conflict, and a post-hoc read shows a single incremented `owner_generation` | `ACTOR_REPORTED` → `INDEPENDENTLY_REPRODUCED` | `NOT STARTED` |
| 4 | Expiry must not let an old owner overwrite a new owner | Fencing-token compare-and-swap independent of lease-expiry clock state (§D) | Same as #3 | **Race/negative test**: simulate an old owner presenting a stale `owner_generation` after a new claimant has superseded it (via injected clock) — assert the stale write is rejected even though the old owner's local view still shows itself as "within lease" | `ACTOR_REPORTED` → `INDEPENDENTLY_REPRODUCED` (this is the single highest-priority test in the whole implementation) | `NOT STARTED` |
| 5 | Idempotent transitions | `idempotency_key` replay/conflict logic (§E) | `transition()`'s idempotency-key lookup path | Test: identical retried request returns identical cached result (no double-apply, `retry_count` unchanged); differing request under the same key is rejected | `ACTOR_REPORTED` → `INDEPENDENTLY_REPRODUCED` | `NOT STARTED` |
| 6 | Bounded, durable retries | Retry counters reusing `MAX_REMEDIATION_CYCLES` (§E) | Retry-counter increment/ceiling-check path | Test: ceiling-exceeded forces `FAILED`/`PAULO_DECISION_REQUIRED`, never an unbounded loop; ceiling value is read live, not hardcoded | `ACTOR_REPORTED` → `INDEPENDENTLY_REPRODUCED` | `NOT STARTED` |
...
3. Is reusing the live `MAX_REMEDIATION_CYCLES` value as the generalized retry ceiling (§E) an appropriate reuse of existing policy, or does the Architect consider task-execution retries (build/QA/review loops) different enough in kind from Architect-remediation cycles that S4 should instead request its own explicitly Paulo-set ceiling (while still never inventing one unilaterally)?
