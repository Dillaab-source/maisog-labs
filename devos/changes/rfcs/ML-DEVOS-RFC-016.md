...
1. attempt `fs.open('<task_id>.lock', 'wx')`; on `EEXIST`, see "Stale-lock recovery" below;
...
**Stale-lock recovery.** A crash while the lock is held would otherwise strand the lock forever. `<task_id>.lock`'s content records `{holder: actor_id, acquired_at: timestamp}`. A writer that fails to acquire the lock (`EEXIST`) reads the existing lock file's `acquired_at`; only if it is older than a fixed, short **lock-staleness ceiling** (an implementation constant bounding how long a critical section may legitimately run — deliberately much shorter than any task's lease duration, and not itself a task-policy value) may it attempt to break the stale lock via its own atomic `unlink` followed immediately by a fresh `wx` create-attempt. Because the unlink-and-recreate attempt can itself race with another writer doing the same thing, the algorithm is a loop — attempt exclusive create, on `EEXIST` check staleness, if stale attempt to steal, retry the create, and either win or lose — never a blind assumption that staleness implies safety to proceed without re-attempting the exclusive create.
...
- **Restart.** The kernel holds no state only in memory; a full process restart simply re-opens the persistence layer and reads whatever was last durably committed, including any orphaned `.lock` file, which the stale-lock recovery mechanism (§D) reclaims. There is no separate "in-flight transition" concept that needs its own recovery procedure beyond re-reading current state — this is a stated design constraint, not an incidental property.
...
| 8 | Crash-safe persistence and mutual exclusion | `wx`-exclusive lock file plus write-temp-then-atomic-rename (§D, §F) | `devos/state/store.mjs` file-backed adapter | **Restart/corruption test**: simulate a leftover `.tmp` file from a crash mid-write — assert the loader ignores it and returns only the last successfully renamed record; simulate a corrupted final file — assert a scoped, explicit error for that one `task_id`, not a process crash. **Stale-lock test**: simulate an orphaned `.lock` file older than the staleness ceiling — assert a new writer can reclaim it and proceed; simulate one younger than the ceiling — assert a new writer correctly waits/retries rather than stealing it prematurely | `ACTOR_REPORTED` → `INDEPENDENTLY_REPRODUCED` | `NOT STARTED` |
