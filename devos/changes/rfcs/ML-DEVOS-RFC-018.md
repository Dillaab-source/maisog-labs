...
5. **A finite publication-attempt limit is mandatory.** V0 sets this bound at a small integer (e.g. 3) the implementation must state explicitly; there is no unbounded or open-ended retry loop under any circumstance.
...
1. **Joint validation, not single-field matching.** STATE's live record and CURRENT_HANDOFF's header must be validated together as one tuple: `handoff_id`, `cycle_id`, `review_target_commit`, and the identity of the applicable Architect review (its own durable/rolling identifier, e.g. an `ML-DEVOS-AS-<NNN>` reference or, while still rolling, its exact reviewed-snapshot commit). A packet where any one of these four disagrees between STATE and CURRENT_HANDOFF is fail-closed as a whole, not accepted on partial agreement.
2. **STATE explicitly names the applicable handoff and the applicable review.** STATE's live record carries both the current `handoff_id` it considers authoritative and the current applicable-review identity (durable ID once archived, or exact reviewed commit while still rolling) as separate, explicit fields — never inferred from which files happen to exist, their timestamps, or their position in a listing.
3. **The current response handoff is distinguished from every referenced prior review.** CURRENT_HANDOFF's `review_target_commit` names the exact commit the *current* handoff responds to; any other Architect review a handoff cites for historical/background context is referenced separately and must never be confused with, or silently substituted for, the one review STATE currently names as applicable. A handoff cannot make an old review current merely by discussing it.
4. **IDs are immutable.** Once a `handoff_id` (or a durable review ID) is published, its bound content (at minimum `cycle_id`, `review_target_commit`, and body) is fixed. A later turn needing a new packet mints a new ID; it never mutates a published one's binding in place.
...
6. **Permitted `input_base_commit` / `review_target_commit` relationships are explicit.** `input_base_commit` records the exact commit the Builder actually read as its working snapshot; `review_target_commit` records the exact commit the resulting handoff is submitted for review against. `review_target_commit` must be a commit reachable from (at or after) `input_base_commit` on the governed branch — a handoff must never name a `review_target_commit` that precedes or diverges from the snapshot it claims to have worked from, since that would let a handoff claim to review work it could not actually have produced from its stated starting point.
7. **An explicit no-handoff state exists.** For a turn where CURRENT_HANDOFF does not apply (e.g. an Architect-authoring turn, a pure design-remediation turn producing only RFC/coordination text, or a turn before any Builder work has occurred in a cycle), STATE explicitly records that no CURRENT_HANDOFF is currently applicable, rather than leaving a stale prior `handoff_id` in place to be misread as still current. A reader must never assume "no handoff mentioned" means "the last one still applies" — the no-handoff state is a named, explicit condition, not an absence to be inferred.

Snapshot-consistent reads (below) depend on this binding: a packet is coherent only when all four bound identifiers agree across STATE and CURRENT_HANDOFF at the snapshot being read.
...
- the complete turn-packet identity binding (`handoff_id`, `cycle_id`, `review_target_commit`, applicable-review identity — per "STATE and CURRENT_HANDOFF relationship," above);
