...
| `requires.evidence` | array of evidence classes | No | The **minimum** evidence class(es) always required for `applies_when`. Never a claim that this is the complete, universal evidence requirement for every claim under this rule — see `requires.evidence_note` and `EVIDENCE_PROVENANCE_MODEL.md`. |
...
`./validate-rules.mjs` (Node, zero dependencies, run manually — not wired into any CI or git hook) parses every `*.json` file in `../rules/` with native `JSON.parse` and checks: valid JSON syntax (fail-closed — a syntax error stops everything, nothing is silently skipped); all required fields present with correct types; `class`/`scope`/`risk`/`status`/evidence-class enums valid; no duplicate `rule_id`; every non-null `supersedes` resolves to an existing `rule_id`; every rule's authority/risk metadata meets its class's minimum; and `status`/`effective_version`/`proposed_effective_version` are mutually consistent. See the script's own header comment for the exact "what this proves / does not prove" list, also reproduced in this cycle's handoff.
