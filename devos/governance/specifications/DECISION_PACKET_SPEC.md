...
| `payload` **XOR** `payload_hash` + `payload_hash_algorithm` | **S1-F006 correction: mutually exclusive, never both, never neither.** Either the exact content of the change (`payload`) with neither hash field present, or a hash of it with a declared algorithm (`payload_hash`/`payload_hash_algorithm`, e.g. `sha256`, both present together) when the content is too large to inline — so the approver is approving *this specific content*, not a description of it, and an unverifiable hash with no declared algorithm is never accepted. **Remediation cycle 2:** an orphaned `payload_hash_algorithm` with no `payload_hash` alongside an exact `payload` is also rejected — the schema's `oneOf` now forbids either hash field, not only `payload_hash`, on the exact-payload branch. |
