...
//
// Durable vs. rolling surfaces (ML-DEVOS-AS-040 / AS40-F001): hard
// missing-canonical-target/duplicate-definition detection is computed only
// from a "durable" subset of the scanned files — `coordination/`'s rolling
// working/turn documents and this subsystem's own directory (which
// necessarily discuss unresolved, proposed, or not-yet-archived IDs while
// describing/reviewing this very tool) are excluded from that subset via
// `scan.workingSurfaceExcludePaths`, to avoid a self-referential feedback
// loop (finding -> review text mentions finding -> scanner reads review ->
// new finding). Canonical-definition discovery is unaffected, since it
// reads its own configured file/dir directly rather than through this
// filtered list. See listDurableReferenceFiles() below.
//
...
// ML-DEVOS-AS-040 / AS40-F001: rolling working/turn surfaces (the live
// coordination handoff/review/state documents) and this subsystem's own
// README/source/config necessarily discuss unresolved, proposed, or
// not-yet-archived IDs as part of describing/reviewing this very tool —
// that is not a durable semantic cross-reference and must not feed hard
// missing-canonical-target/duplicate-definition detection (doing so would
// create a self-referential feedback loop: finding -> review text mentions
// finding -> scanner reads review -> new finding). This filters the
// already-scanned file list down to the subset used for that hard
// referential-integrity check; canonical-definition discovery is
// unaffected because it reads its own configured file/dir directly (see
// discoverDefinitionsForFamily above), never through this list.
function isWorkingSurfaceFile(relPath, workingSurfaceExcludePaths) {
  return workingSurfaceExcludePaths.some(prefix => relPath === prefix || relPath.startsWith(prefix));
}

export function listDurableReferenceFiles(scannedFiles, config) {
  const workingSurfaceExcludePaths = config.scan.workingSurfaceExcludePaths ?? [];
  return scannedFiles.filter(file => !isWorkingSurfaceFile(file, workingSurfaceExcludePaths));
}
...
export function buildTraceabilityReport(repoRoot = REPO_ROOT, config = loadConfig()) {
  ...
  const durableReferenceFiles = listDurableReferenceFiles(scannedFiles, config);
  ...
  for (const family of [...config.idFamilies].sort((a, b) => a.family.localeCompare(b.family))) {
    ...
    // Reference extraction reads only durable surfaces (AS40-F001) — every
    // occurrenceCount/inboundReferenceCount/orphan/missing-target/
    // duplicate-definition computation below is therefore already
    // durable-only; coordination/ and this subsystem's own files can never
    // feed a hard finding.
    const references = extractReferencesForFamily(repoRoot, family, durableReferenceFiles);
    ...
    for (const id of definitionIds) {
      ...
      if (inbound.length === 0) {
        warnings.push({
          ...
          message: `${id} is canonically defined but has no inbound reference anywhere else in the durable scanned surface.`,
        });
      }
      idEntries.push({
        id,
  ...
  return {
    ...
    durableReferenceScannedFileCount: durableReferenceFiles.length,
    workingSurfaceExcludePaths: config.scan.workingSurfaceExcludePaths ?? [],
    idFamilies: families,
    summary: {
...
export function renderMarkdown(report) {
  ...
  lines.push("");
  lines.push(`Scanned files: ${report.scannedFileCount} (${report.durableReferenceScannedFileCount} used for durable reference/error detection; rolling working surfaces excluded from hard findings: ${report.workingSurfaceExcludePaths.map(p => `\`${p}\``).join(", ")})`);
  lines.push("");
