...
- **Implementation status:** `IMPLEMENTED` at the repository level (local-only; `ML-DEVOS-RFC-003` → `ML-DEVOS-AS-013` → `D-024`) — not yet Architect-verified. See `brain/GOVERNANCE_MAP.md`, `brain/TEST_LEDGER.md`, and `coordination/IMPLEMENTER_HANDOFF.md` for evidence. The public site continues to read only `data/site.js`; no public cutover occurred.
...
- **Likely change class:** `CAPABILITY` (a new authenticated read capability against the protected substrate), contingent on `WEB-INC-005`'s `ARCHITECTURE`-class substrate already existing. **Actual classification determined at authorization:** `ARCHITECTURE` (`ML-DEVOS-RFC-004`/`ML-DEVOS-AS-015`), because the grounded proposal composed the WEB-INC-001 identity boundary with the WEB-INC-005 D1 subsystem for the first time, not merely enabling an already-defined tool.
...
- **Implementation status:** `IMPLEMENTED` at the repository level (local-only; `ML-DEVOS-RFC-004` → `ML-DEVOS-AS-015` → `D-025`) — not yet Architect-verified. Exactly one endpoint, `GET /admin/api/dashboard`, returns an allowlisted status projection for all seven current-content domains; authentication precedes all routing/D1 access; the 14-table WEB-INC-005 schema is unchanged. See `brain/GOVERNANCE_MAP.md`, `brain/TEST_LEDGER.md`, and `coordination/IMPLEMENTER_HANDOFF.md` for evidence. The public site continues to read only `data/site.js`; no public cutover, mutation capability, or remote D1 resource was introduced.
...
- **Likely change class:** `CAPABILITY` (adds a new, low-risk-but-sensitive write path — an append-only log). **Actual classification determined at authorization:** `ARCHITECTURE` (`ML-DEVOS-RFC-005`/`ML-DEVOS-AS-017`), because this is the first persistent application-side D1 write primitive and the first append-only-history schema pattern, elevating it past a generic capability addition (`AS17-F001`).
...
- **Implementation status:** `ARCHITECT_APPROVED` at the repository/local level (`ML-DEVOS-RFC-005` → `ML-DEVOS-AS-017` → `D-026` → `ML-DEVOS-AS-018` remediation → `ML-DEVOS-AS-019`; architecture recorded by `ML-DEVOS-ADR-005`). Exactly one new table, `audit_log` (current schema: 15 tables total — the 14 `WEB-INC-005` tables, unchanged, plus this one); append-only enforced at both the application layer (`worker/d1/audit.mjs`'s `appendAuditEvent` exposes no update/delete helper) and the database layer (`migrations/0002_web_inc_008_audit_log.sql`'s `BEFORE UPDATE`/`BEFORE DELETE` triggers, independently confirmed via direct `wrangler d1 execute --local`); no HTTP audit API/UI, no editorial mutation capability, and no admin action produces a row yet, since none exists. See `brain/GOVERNANCE_MAP.md`, `brain/TEST_LEDGER.md`, and `coordination/IMPLEMENTER_HANDOFF.md` for evidence.
...
- **Actual classification determined at authorization:** `CAPABILITY` (`ML-DEVOS-RFC-006`/`ML-DEVOS-AS-020`) — composes already-accepted architecture (verified Access identity → project revision substrate → audit substrate) into the first authenticated editorial write capability; no new table/storage subsystem.
- **Implementation status:** `ARCHITECT_APPROVED` at the repository/local level (`ML-DEVOS-RFC-006` → `ML-DEVOS-AS-020` → `D-027` → `ML-DEVOS-AS-021` remediation → `ML-DEVOS-AS-022`). No ADR is required for this `CAPABILITY`-class change; the future capability registry is not implemented yet. Exactly five new protected routes (`POST /admin/api/projects`, `PUT /admin/api/projects/:id/draft`, `GET /admin/api/projects/:id/preview`, `POST /admin/api/projects/:id/publish`, `POST /admin/api/projects/:id/unpublish`); mutation requires a bounded non-empty verified Access `sub` (a service-token-style empty-sub assertion cannot mutate); every mutating route enforces same-origin/JSON/bounded-body request hardening; create/edit each write one new immutable `project_revisions` row (no existing revision content row is ever `UPDATE`d); slug is immutable; existing-project mutations require `expectedPublishedRevisionId`/`expectedDraftRevisionId` and fail closed with `409` on stale state; publish fully revalidates the persisted draft before promotion; every successful state-changing mutation and its `audit_log` success row commit as one `db.batch()` transaction, proven both by a real-D1 test that forces the audit statement to fail and confirms the project/revision insert rolls back with it, and by an HTTP-level simulated storage-failure test; bounded authenticated failures (validation/not-found/conflict/stale/revalidation) record a `result: failure` audit row when a safe entity reference exists. Schema unchanged — still exactly 15 product tables; `migrations/0001_web_inc_005_init.sql` and `migrations/0002_web_inc_008_audit_log.sql` untouched. No project delete, no generic mutation API, no admin UI change, no remote D1, no public D1 cutover, no deployment. See `brain/GOVERNANCE_MAP.md`, `brain/TEST_LEDGER.md`, and `coordination/IMPLEMENTER_HANDOFF.md` for evidence.
...
- **Implementation status:** `ARCHITECT_APPROVED` at repository/local level (`ML-DEVOS-RFC-007` → `ML-DEVOS-AS-023` → `D-029` → initial implementation `ca6a93b...` → `ML-DEVOS-AS-026` remediation → `681fc90...` → `ML-DEVOS-AS-027`; architecture recorded by `ML-DEVOS-ADR-007`). Exactly 17 product tables; local R2 simulation only; no public media serving, remote R2/D1, deployment, or main merge. Accepted limitation `AS27-L001` records SQLite `trim()` being narrower than JS trim for hypothetical direct-SQL edge cases.


### `WEB-INC-006` — Journal (owns `journal_entries`/`journal_entry_revisions`/`journal_media` — `AS10-R009`)
- **Requirements:** `ADM-REQ-005`; `WEB-REQ-009` (public published-only Journal browsing, owned by `PRD.md`).
- **Design refs:** `APP_FLOW.md` §1c; `DATA_BACKEND_SPEC.md` § `journal_entries`/`journal_entry_revisions`, § `journal_media`; `ML-DEVOS-RFC-009`.
- **Change class:** `ARCHITECTURE` — the increment added three persistent tables, a new public content type, a published-only public Worker→D1 read path, and widened Worker-first routing only for `/api/journal` and `/api/journal/*`.
- **Accepted scope:** exactly `journal_entries`, `journal_entry_revisions`, `journal_media`; protected create/edit/preview/publish/unpublish Journal lifecycle; Journal dashboard status; published-only `GET /api/journal` and `GET /api/journal/:slug`; static `/journal` shell; plain-text body; revision-scoped media snapshots; bounded audit actions.
- **Implementation status:** `ARCHITECT_APPROVED` at repository/local level. Authority chain: `WEB-REQ-009 → ML-DEVOS-RFC-009 → ML-DEVOS-AS-028 → D-031 → cdc8f84... → ML-DEVOS-AS-029 → ML-DEVOS-ADR-008`. Schema target reached: exactly 20 product tables. Builder reported Journal admin `44/44`, public Journal `16/16`, full suite `269/269`, and successful static build. No remote D1/R2, deployment, main merge, homepage/projects D1 cutover, or public media-object serving is accepted or claimed.
- **Accepted limitations:** `AS29-L001` base-row immutable metadata lacks a dedicated DB freeze trigger; `AS29-L002` Journal audit actions are fixed at call sites rather than a global literal action enum; `AS29-L003` hypothetical direct SQL can choose the first non-null `published_at` value. No direct-DB writer is authorized.
