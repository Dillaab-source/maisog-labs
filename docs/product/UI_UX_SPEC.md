...
The `DESIGN-001`…`014` catalog (`docs/MAISOGLABS_WEBSITE_GOVERNANCE_ADMIN_PLAN_v0.1.txt` §10) already names the intended admin-exposed design controls (hero background/media, section visibility/order, card style, layout density, typography preset, heading scale, overlay intensity, glass/panel preset, animation preset, reduced-motion compatibility, project-rail/journal-card behavior, constrained theme tokens). This spec does not restate their descriptions; it records that all fourteen remain `NOT STARTED` today (`brain/GOVERNANCE_MAP.md`) and that any future control must stay within Brand V3's non-negotiables (`brand/V3/DESIGN_MAP.md` § "Non-negotiables": no logo redesign, no generic sci-fi/AI-brain stock art, no bright cyberpunk palette, no UI baked into the background image, no major re-composition without approval).
