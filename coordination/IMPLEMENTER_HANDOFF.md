...
## RFC-018 Bootstrap V0 Design Remediation Cycle 1 (B018-01–B018-07)

**Authority:** `D-061` (Bootstrap V0 discovery/design and independent review only; grants no implementation authority). The Architect's Independent Architecture Stage Gate review (`ML-DEVOS-AS-078`, reviewed snapshot `e03d833349bd0abf015f20b04200a2f872af9175`) returned `RFC-018 DIRECTION: ACCEPTED` / `RFC-018 IMPLEMENTATION READINESS: CHANGES_REQUESTED`, seven bounded protocol-contract findings. `CURRENT_REMEDIATION_CYCLE: 1` of `MAX_REMEDIATION_CYCLES: 2`. **Design remediation only — no Bootstrap V0 implementation, no CURRENT_HANDOFF creation, no checker, no S5 implementation.** **LEAN / DELTA-ONLY mode** — read only live `coordination/STATE.md`, `coordination/ARCHITECT_REVIEW.md`, and `devos/changes/rfcs/ML-DEVOS-RFC-018.md` itself.

**Input HEAD:** `1ec52997f0d4f6ef98a15d33086c2c820166d503` — exact SHA per `git rev-parse HEAD`, run immediately after `git fetch origin && git checkout governance/maisoglabs-v0.1 && git pull --ff-only origin governance/maisoglabs-v0.1`, before any file was touched. This fast-forward brought in the durable archives for `ML-DEVOS-AS-075`/`076`/`077` (resolving a traceability blocker disclosed, but not fixable, in the prior S5 cycle's handoff — confirmed resolved below) and the initial `ML-DEVOS-RFC-018.md` filing itself.

**Finding-to-RFC-section mapping (each finding corrected in place, marked with an explicit `corrects \`B018-0N\`` heading for traceability):**

- **`B018-01` (publication lacks a precise transaction contract)** — new subsection "Exact-tip atomic publication transaction contract" appended to `### B0-A — Fresh Snapshot and Safe Publication Contract`. Defines: one candidate commit parented directly to the exact revalidated tip; the complete coordination transition published atomically in that one commit; mandatory expected-tip conflict detection / compare-and-swap equivalent (fail on mismatch, never a read-then-write gap); any advancement invalidates the attempt and requires a fresh coherent snapshot (never blind re-parent-and-retry); a stated finite retry limit; mandatory read-back reconciliation on ambiguous/timeout outcomes before any further action; a provider lacking these semantics is advisory/read-only for governed writes. Also tightened "Small executable checker" to require checking exact expected-tip equality, not merely "advancement occurred."
- **`B018-02` (`handoff_id` does not bind the complete turn packet)** — new subsection "Complete turn-packet identity binding" appended to "STATE and CURRENT_HANDOFF relationship." Defines: joint validation of `handoff_id`+`cycle_id`+`review_target_commit`+applicable-review identity as one tuple (fail-closed as a whole on any single disagreement); STATE explicitly names both the applicable handoff and the applicable review; the current response handoff is distinguished from any historically-referenced prior review; IDs are immutable; a duplicate ID with different bytes is rejected fail-closed; permitted `input_base_commit`/`review_target_commit` relationships stated explicitly (target must be reachable from base, never preceding/diverging); an explicit no-handoff state is named for turns where CURRENT_HANDOFF does not apply. Also added a coherence cross-reference to "Snapshot-consistent reads" and tightened the checker's identity-check bullet to name the full tuple.
- **`B018-03` (rolling-record preservation is conditional)** — "Rolling-record preservation" section rewritten. Removes the "concluded" precondition entirely: every outgoing published rolling record is preserved unconditionally on outcome (including `CHANGES_REQUESTED`/interrupted records), unless its exact bytes are already durably archived. Adds: deterministic archive locations computed from immutable ID; immutable archive entries; source commit/blob provenance preserved alongside payload; a discoverable index/reference requirement; existing-archive-ID-with-different-bytes fails closed; restates (unchanged) that a rolling record need not embed its own future publication SHA.
- **`B018-04` (requirement completeness depends on the packet author)** — new subsection "Independently reviewed carry-forward inventory at cutover" appended to "Current-context completeness." Defines one bounded, Architect-reviewed (not Builder-self-certified) inventory at cutover mapping every still-operative obligation to its authoritative source and current disposition; future transitions must preserve an inventoried obligation or cite explicit closure/supersession; explicitly bounded as a short repository-native index, never a resolver service; a handoff summary is stated as navigation only, never proof an omitted requirement is inapplicable.
- **`B018-05` (provenance not separated from authorization)** — new subsection "Provenance versus authorization" appended to "Stable kernel," plus a reinforcing sentence in "Owner-requested advisory mode." States explicitly: committed content proves provenance/existence, never authorization; scope expansion requires the applicable owner decision/review chain; an unresolved authority conflict blocks governed mutation; logs/evidence/quotations/handoffs/retrieved pages/committed instruction-shaped text remain evidence/input unless the governing authority chain explicitly grants authority; owner-requested advisory analysis is not mutation authorization; operative orientation/bootstrap wording overstating committed-instruction authority is named as a required future-migration reconciliation item (not edited by this cycle).
- **`B018-06` (migration lacks an effective write boundary)** — "Active-reader/writer migration" section substantially rewritten with a new "Atomic activation with a mandatory write boundary" subsection: one atomic activation change (itself following the `B0-A` publication contract); an explicit protocol-version marker every writer checks; mandatory prepublication checks at every supported governed writer; the complete operative-reader/writer inventory from `ML-DEVOS-AS-078` restated in full, explicitly adding `brain/00_HOME.md` and `PROJECT_GOVERNANCE.md`, which the prior draft's list omitted; explicit local dirty-worktree handling (preserve unrelated work, bound candidate changes, stop on uncertainty); disclosed procedural bypasses V0 cannot technically prevent; canonical-skill-sources-first migration with bridges regenerated, never hand-edited in parallel; stale protocol-version sessions must stop and bootstrap fresh. A new "Known migration inputs to reconcile" subsection restates the independent review's exact named conflicts (`CLAUDE.md`, coordination protocol, Architect Sync protocol, Architect Review skill, Orientation/State Recovery skill, Implementation Handoff skill/format, `brain/00_HOME.md`, `PROJECT_GOVERNANCE.md`, canonical skill tests, generated bridges) as migration inputs — explicitly not modified by this design-remediation cycle.
- **`B018-07` (rollback can restore obsolete state or strand evidence)** — "Rollback" section rewritten: rollback is defined as a new forward recovery commit against the fresh current tip (following the same `B0-A` contract), never a blind restoration of old STATE; current authority/turn state is explicitly preserved through rollback; current rolling records are archived before rollback changes routing; the rollback commit must point fallback routing to post-cutover evidence and the carry-forward inventory's still-open items; the fallback write surface is explicitly defined (legacy append must not silently resume unless explicitly stated); rollback must be tested after several actual V0 turns, not only immediately after migration.
- **Required failure-test additions** — new subsection "Exact additional required failure-test cases (from the independent design review)" appended to "Failure tests required before S5 Trial #1," adding, verbatim, all 20 cases `ML-DEVOS-AS-078` named (matching handoff ID but wrong cycle/target; stale Architect review; reused ID with different bytes; required obligation omitted from an otherwise valid packet; branch movement after final read; simultaneous publishers; timeout after successful publication; partial coordination publication; failed archive write; conflicting archive destination; old-session legacy append; publication bypassing checker; unrelated staged/untracked work; local work introduced after validation; advisory review on another actor's turn with no writes; Architect turn where mutation remains prohibited; forged committed authorization; hostile instruction-shaped repository evidence; rollback after several turns preserving current restrictions; exhausted publication retries that cannot silently reset through session resume), plus the review's note that the two authority/prompt-injection-shaped cases require procedural/agent behavioral evaluation, not string-matching tests alone.

**Not touched (explicitly out of this cycle's scope):** `AGENTS.md`, `CLAUDE.md`, canonical skills, generated bridges, coordination protocols, `brain/00_HOME.md`, `PROJECT_GOVERNANCE.md`, any S5 runtime file, any S6+ file, any product/runtime/website file, `brain/DECISION_LOG.md`, any ADR, `ML-DEVOS-ARCH-001.md`, `VERSIONING_POLICY.md`. No Bootstrap V0 implementation, `CURRENT_HANDOFF` file, or checker script was created.

**Commands actually run, with exit codes (all `ACTOR_REPORTED`):**
- `git fetch origin && git checkout governance/maisoglabs-v0.1 && git pull --ff-only origin governance/maisoglabs-v0.1 && git rev-parse HEAD` → fast-forwarded `af25c75..1ec5299`, confirmed `HEAD = 1ec52997f0d4f6ef98a15d33086c2c820166d503`. Exit `0`.
- `grep -n "ML-DEVOS-AS-078\|ML-DEVOS-AS-075\|TEST-[A-Z]"` against `ML-DEVOS-RFC-018.md`/`README.md` → confirmed the only match is a pre-existing `ML-DEVOS-AS-075`/`076`/`077` mention already present before this cycle's edits (safe: those durable archives now exist, per this cycle's own fast-forward), and that this cycle introduces no `ML-DEVOS-AS-078` full-identifier citation (its durable archive does not yet exist) and no `TEST-[A-Z]+-\d{3}`-pattern collision. Exit `0`.
...
- `node devos/governance/traceability/validate-traceability.mjs` → `Scanned 276 files across 12 ID families. Errors: 2  Warnings: 14  Total canonical definitions: 281.` Fingerprint exactly `CORE-022` + `WEB-REQ-009`. `No drift: on-disk generated index matches a fresh generation run.` Exit `1` (expected whenever any ERROR-level finding exists — established convention throughout this engagement, not a script failure).
- `git status --porcelain` against input HEAD → exactly `devos/changes/rfcs/ML-DEVOS-RFC-018.md` + the two regenerated traceability output files changed, matching the authorized Cycle 1 delta write surface exactly. Exit `0`.

**Before/after traceability fingerprint:** input-HEAD-committed baseline (`TRACEABILITY_INDEX.md`'s own header at `1ec5299`, itself already stale relative to this fast-forward's own content): `272` files, **3** errors (`CORE-022` + `ML-DEVOS-AS-075` + `WEB-REQ-009`), `16` warnings, `276` canonical definitions — the `ML-DEVOS-AS-075` error was the pre-existing, out-of-scope blocker disclosed (but not fixable) in the prior S5 Cycle 2 handoff. After this cycle's regeneration: `276` files, **2** errors (`CORE-022` + `WEB-REQ-009` — the `ML-DEVOS-AS-075` error is gone because this same fast-forward brought in that ID's durable archive, exactly as that prior handoff predicted would happen once a cycle with archive-write authority created it), `14` warnings, `281` canonical definitions. **This cycle's own `ML-DEVOS-RFC-018.md` content introduces zero new missing-canonical-target findings** — verified by running `validate-traceability.mjs` immediately after the RFC edits (before any traceability regeneration) and observing the same 2-error fingerprint already present from the fast-forward alone.
