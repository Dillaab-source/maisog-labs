...
const MUTANTS = [
  ...
  },
  {
    ...
    file: "devos/execution/host.mjs",
    from: 'if (s && s.state === "ISSUED" && clock() >= s.claim_deadline_ms) return',
    to: 'if (s && (s.state === "ISSUED" || s.state === "CLAIMED") && clock() >= s.claim_deadline_ms) return',
    tests: "tests/execution-permits.test.mjs",
  ...
  // ---- AS95 remediation mutants
  {
    ...
    file: "devos/execution/host.mjs",
    from: "  function locked(taskId, fn) {\n",
    to: "  function locked(taskId, fn) {\n    if (taskId) return lockScope.run(new Set([taskId]), fn);\n",
    tests: "tests/execution-linearization.test.mjs",
    pattern: "simultaneous claim and quiesce serialize",
  },
  {
    id: "M25-no-permit-version-cas",
    file: "devos/execution/registry.mjs",
    from: '      if ((status.version ?? 0) !== (current.version ?? 0)) fail("ISOLATION_UNPROVABLE", `stale write to permit ${permitId} rejected (version CAS)`);\n',
    to: "",
    tests: "tests/execution-linearization.test.mjs",
    pattern: "stale or illegal writes are refused",
  },
  {
    id: "M26-no-instance-version-cas",
    file: "devos/execution/registry.mjs",
    from: '      if ((record.version ?? 0) !== (current.version ?? 0)) fail(',
    to: '      if (false) fail(',
    tests: "tests/execution-linearization.test.mjs",
    pattern: "stale or illegal writes are refused",
  },
  {
    ...
    file: "devos/execution/registry.mjs",
    from: '      if (!(PERMIT_NEXT[current.state] ?? []).includes(status.state)) fail(',
    to: '      if (false) fail(',
    tests: "tests/execution-linearization.test.mjs",
    pattern: "stale or illegal writes are refused",
  },
  ...
  },
  {
    ...
    file: "devos/execution/host.mjs",
    from: '      if (status.state === "EXPIRED_UNCLAIMED" && status.expired_derived) putStatusLocked(record, binding, status, { state: "EXPIRED_UNCLAIMED" });\n',
    to: "",
    tests: "tests/execution-linearization.test.mjs",
