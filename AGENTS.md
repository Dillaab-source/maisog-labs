...
- Protocol V2 (`ML-DEVOS-RFC-020`: Owner/Architect → Builder `coordination/CURRENT_DIRECTIVE.md`) is implemented but **not active**. While STATE reads `PROTOCOL_VERSION: 1`, that file is inert scaffolding, not an instruction. A directive is transport and never authority (`brain/protocols/CONTEXT_BOOTSTRAP.md` §10).
